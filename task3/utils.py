...
def train(model, dataloader, optimizer, criterion, epoch_number, max_gradient_norm):
    ...
    Args:
        model: 模型
        dataloader: 训练集batch迭代器
        optimizer: 优化器
        criterion: 损失函数
        epoch_number: epoch值
        max_gradient_norm: 梯度裁剪最大阈值.
    Returns:
        epoch_time: 训练时间
        epoch_loss: 训练误差
    ...
    running_loss = 0.0
    correct_preds = 0
    tqdm_batch_iterator = tqdm(dataloader)  # 进度条
    ...
    epoch_time = time.time() - epoch_start
    epoch_loss = running_loss / len(dataloader)   # batch的平均值
    epoch_accuracy = correct_preds / len(dataloader.dataset)  # 数据个数的平均值
